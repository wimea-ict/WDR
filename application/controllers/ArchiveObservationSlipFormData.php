...
class ArchiveObservationSlipFormData extends CI_Controller {
    ...
    public function index($id=NULL){
        ...
        $query = $this->DbHandler->selectAll($userstation,'StationName','archiveobservationslipformdata','','',$id,$userid);
        if ($query) {
         ...
         if($id!=NULL){
         $data['observationslipcomments'] = $this->DbHandler->selectRecordComments($id,'archiveobservationslipformdata');
        }
        ...
        $form_type="archiveobservationslipformdata";
        ...
        if($insertsuccess){
            ...
            $q= "update archiveobservationslipformdata set numberofcomments=numberofcomments+1 where id='$recordid'";
    ...
    public function markcommentAsResolved($id){
         $q= "update raw_datacomments set solved='TRUE' where id='$id'";
        $updatesuccess= $this->DbHandler->updatesubmittedreports($q);
        if($updatesuccess){
            $this->session->set_flashdata('success', 'Comment marked successfully!');
        }else{
            $this->session->set_flashdata('success', 'Operation failed!');
        }
        redirect($_SERVER['HTTP_REFERER']);
    }
    public function archiverainfallform(){
        //$this->unsetflashdatainfo();
        $session_data = $this->session->userdata('logged_in');
        //$userrole=$session_data['UserRole'];
        $userstation=$session_data['UserStation'];
        ...
        $this->load->view('archiverainfallform', $data);
    }
    ...
    public function DisplayNewArchiveRainfallForm(){
        $this->unsetflashdatainfo();
        $name='displaynewarchiveobervationslipform';
        $data['displaynewarchiveobervationslipform'] = array('name' => $name);
    ...
    public function DisplayArchiveRainfallFormForUpdate(){
        $this->unsetflashdatainfo();
        $session_data = $this->session->userdata('logged_in');
        $userrole=$session_data['UserRole'];
        $userstation=$session_data['UserStation'];
    ...
    public function DisplayArchiveObservationSlipFormForValidation(){
        $this->unsetflashdatainfo();
        $session_data = $this->session->userdata('logged_in');
        $userrole=$session_data['UserRole'];
        $userstation=$session_data['UserStation'];
        ...
        $query = $this->DbHandler->selectById($archivedobservationslipformdataid,'id','archiveobservationslipformdata');  //$value, $field,$table
        if ($query) {
            $data['archivedobservationslipformdataidvalidate'] = $query;
        } else {
            $data['archivedobservationslipformdataidvalidate'] = array();
        }
    ...
    public function insertArchiveObservationSlipFormData(){
       ...
       // $creationDate= date('Y-m-d H:i:s');
        ...
        $insertObservationSlipFormData=array(
         ...
         $checkduplicateform = $this->DbHandler->checkforduplicatearchiveobservationslip($date,$station_id,$timeobservationslipform);

            if($checkduplicateform){
            $this->session->set_flashdata('error', 'Sorry, A Record for this time and date has Already Been Submitted');
                $this->index();
            }
            else{
        ...
        if($insertsuccess){
            ...
            $userlogs = array('Userid' => $userid,
                'Action' => 'Added Archive Observation Slip',
        ...
        if($updatesuccess){
            ...
            $name=$session_data['FirstName'].' '.$session_data['SurName'];
             ...
             if($doqa=="qa"){
                ...
                'Action' => 'Quality control Archive Observation Slip',
                'Details' => $name . ' did quality assurance checks on Archive Observation Slip Form info into the system',
                'IP' => $this->input->ip_address());
               
              }else{
                $userlogs = array('Userid' => $userid,
                'Action' => 'Updated Archive Observation Slip',
    ...
    public 	function update_approval() {
	  ...
	  $user_id=$session_data['Userid'];
       $name=$session_data['FirstName'].' '.$session_data['SurName'];
		$id= $this->input->post('id');
		$data = array('Approved' => $this->input->post('approve'),'ApprovedBy'=>(($this->input->post('approve')=="FALSE")? "":$name));
    ...
    public function deleteArchiveObservationSlipFormData() {
        ...
        if ($rowsaffected) {
            ...
            $userlogs = array('Userid' => $id,
                'Userid' => $userid ,'Action' => 'Deleted Archive Observation Slip',
