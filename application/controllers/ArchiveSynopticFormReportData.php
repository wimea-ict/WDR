...
class ArchiveSynopticFormReportData extends CI_Controller {
    ...
    public function index($id=NULL){
       // $this->unsetflashdatainfo();
        ...
        $query = $this->DbHandler->selectAll($userstation,'StationName','archivesynopticformreportdata','','',$id,$userid);  //value,field,table
        ...
        if($id!=NULL){
         $data['observationslipcomments'] = $this->DbHandler->selectRecordComments($id,'archivesynopticformreportdata');
        }
        ...
        $form_type="archivesynopticformreportdata";
        ...
        if($insertsuccess){
            ...
            $q= "update archivesynopticformreportdata set numberofcomments=numberofcomments+1 where id='$recordid'";
    ...
    public function DisplaySynopticFormForUpdate(){
        ...
        public function DisplaySynopticFormForValidation(){
        $this->unsetflashdatainfo();
        //Get all Stations.
        $session_data = $this->session->userdata('logged_in');
        $userrole=$session_data['UserRole'];
        ...
        // $query = $this->DbHandler->selectById('daily','id',$dailyformid);
        if ($query) {
            $data['synopticformdatavalidate'] = $query;
        } else {
            $data['synopticformdatavalidate'] = array();
        }
         $data['regions']=$this->DbHandler->selectStations();
        $this->load->view('archiveSynopticFormReportData', $data);
    }
...
$from = $this->input->post('from');
       ...
       // $creationDate= date('Y-m-d H:i:s');
        ...
        $insertSynopticFormData=array(
            ...
            'Actual_Rainfall'=>$actualrainfall,'submitedBy_Id'=>$userid);

         $checkduplicateform = $this->DbHandler->checkforduplicatearchivesynoptic($date, $stationId,$ztime);
            

            if($checkduplicateform){
                $this->session->set_flashdata('error', 'Sorry, A Record for this time and date Already Exists');
                $this->index();
            }
            else{
    ...
    public function updateSynopticFormData(){
        ...
        $updateSynopticFormData=array(
            ...
            'Fog'=>$fog,
            'EarthQuake'=>$earthquake,'Anemometer_Reading'=>$anemometerReading,
            'Actual_Rainfall'=>$actualrainfall,'qaBy'=>$qao);
        ...
        if($updatesuccess){
           ...
           if($doqa=="qa"){
            ...
            'Action' => 'Quality control Archive Synoptic',
            'Details' => $name . ' did quality assurance checks on Archive Synoptic Form info into the system',
            'IP' => $this->input->ip_address());
           
          }else{
            $userlogs = array('Userid' => $userid,
            'Action' => 'Update Archive Synoptic',
...
//Store User logs.
            ...
            $userlogs = array('User' => $name,
                'Userid' => $userid,'Action' => 'Deleted Archive Synoptic',
