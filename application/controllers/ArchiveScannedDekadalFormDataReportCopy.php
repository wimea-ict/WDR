...
class ArchiveScannedDekadalFormDataReportCopy extends CI_Controller {
    ...
    public function index($id=NULL){
        ...
        $query = $this->DbHandler->selectAllScansmonthly($userstation,'StationName','scans_dekadals','',$userid,$id);  //value,field,table
        //  var_dump($query);
        if ($query) {
        ...
        $data['uploaded_files'] = $this->DbHandler->archievescanned_files();
         if($id!=NULL){
         $data['observationslipcomments'] = $this->DbHandler->selectRecordComments($id,'scans_dekadals');
        }
    ...
    public function submitObservationslipComment(){
        ...
        if($insertsuccess){
            ...
            $q= "update scans_dekadals set numberofcomments=numberofcomments+1 where id='$recordid'";
    ...
    public function insertInformationForArchiveScannedDekadalFormFiles(){
        $this->unsetflashdatainfo();
        ...
        $insertScannedObservationfiles=array('file' => $single_filename, 'description' => $description,'record_id' => $record);
         ...
         $q="Update scans_dekadals set numfiles=numfiles+1 where record_id='$record'";
         $insertsuccess= $this->DbHandler->updatesubmittedreports($q);  
         header('location:'.$_SERVER['HTTP_REFERER']);
        }
    public function DisplayFormToArchiveScannedDekadalFormFIles($id=NULL){
        $this->unsetflashdatainfo();
        ...
        $file_element_name = 'archievescannedcopy_dekadalformdatareport';
           ...
           // $creationDate= date('Y-m-d H:i:s');
            ...
            $stationId=$this->DbHandler->identifyStationById($station, $stationNumber);//station name and station number
             $record_identity=$stationNumber."dekadal_".date("Ymdhisa");

            $insertScannedDekadalFormReportDataDetails=array(
                'station' => $stationId,
                'record_id'=>$record_identity,
                'Approved'=> $Approved,'SDE_SubmittedBy'=>$SubmittedBy,
                'Description'=>$description,'FileRef' => $single_filename, 'Dekadalnumber'=>$dekadalnumber,'month'=>$month,'year'=>$year,'submitedBy_Id'=>$userid);
                
            $duplicate=$this->DbHandler->checkforDuplicatearchivescanned("Select * from scans_dekadals where year = $year and month = '$month' and Dekadalnumber='$dekadalnumber' and station = $stationId and numfiles>0");
                if( $duplicate>0){
                    $this->session->set_flashdata('error', ' Archive Scanned dekadal '.$dekadalnumber .' record of '.$month.', '.$year.' for the station '.$station. ' already exists!');
                    redirect("ArchiveScannedDekadalFormDataReportCopy");
                }else{
            ...
            if($insertsuccess){
               ...
               $userlogs = array('Userid' => $userid,
                   'Action' => 'Added Archive Scanned Dekadal',
                ...
                redirect("ArchiveScannedDekadalFormDataReportCopy/DisplayFormToArchiveScannedDekadalFormFIles/".$record_identity);
            }
            else{
       ...
       // $config['max_size'] = '2GB';
        ...
        else
        {
            ...
            $updateScannedDekadalFormReportDataDetails=array(
                'station' => $stationId,'Approved'=>$approved, 'comment'=>$comment, 
                'Description'=>$description, 'SDE_SubmittedBy'=>$UpdatedBy, 'FileRef' => $filename, 'Dekadalnumber' => $dekadalnumber, 'month'=>$month,'year'=>$year);
    ...
    public function deleteInformationForArchiveScannedDekadalFormReport() {
        ...
        if ($rowsaffected) {
            ...
            $userlogs = array('User' => $name,
                'UserRole' => $userrole,'Action' => 'Deleted Archive Scanned Dekadal',
...
//check($value,$field,$table)
    ...
    function confirmSubmission($record){
        $query="update scans_dekadals set submissionstatus='Completed' where record_id='$record'";
        $result=$this->DbHandler->updatesubmittedreports($query);
        redirect(base_url('index.php/ArchiveScannedDekadalFormDataReportCopy'));
     } 
